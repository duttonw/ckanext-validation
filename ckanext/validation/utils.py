...
def validate_resource(context, data_dict, new_resource=False):
    create_mode = settings.get_create_mode_from_config()
    update_mode = settings.get_update_mode_from_config()

    mode = create_mode if new_resource else update_mode

    if mode == settings.SYNC_MODE:
        run_sync_validation(data_dict)
    elif mode == settings.ASYNC_MODE:
...
def run_sync_validation(resource_data):
    ...
    report = jobs.validate_table(source, _format=_format, schema=schema, http_session=_get_session(resource_data), **options)
...
def _get_session(resource_data):
    ...
    pass_auth_header = tk.asbool(
        tk.config.get(settings.PASS_AUTH_HEADER, settings.PASS_AUTH_HEADER_DEFAULT))

    if dataset[u'private'] and pass_auth_header:
        _session = requests.Session()
        _session.headers.update({
            u'Authorization':
            tk.config.get(settings.PASS_AUTH_HEADER_VALUE, get_site_user_api_key())
        })
...
def is_resource_could_be_validated(context, data_dict):
    ...
    for plugin in plugins.PluginImplementations(IDataValidation):
        if not plugin.can_validate(context, data_dict):
            log.debug('Skipping validation for resource {}'.format(context['package'].resources[-1].id))
            log.debug('Skipping validation for new resource')
            return False

    if not data_dict.pop('_skip_next_validation', None):
        return False
    ...
    supportable_format = res_format in settings.get_supported_formats()
...
def is_resource_requires_validation(context, old_resource, new_resource):
    ...
    if is_format_changed and new_format in settings.get_supported_formats():
...
def should_remove_unsupported_resource_validation_reports(res_dict):
    if not settings.is_cleanup_reports():
        return False
...
def remove_unsupported_resource_validation_reports(resource_id):
    ...
    if should_remove_unsupported_resource_validation_reports(res):
        ...
        try:
            tk.get_action('resource_validation_delete')(context, {
                "resource_id": res['id']})
...
def get_resource_validation_options(resource_data):
    ...
    """
    options = settings.get_default_validation_options()
